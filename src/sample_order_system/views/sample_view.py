...
_COLS = {"id": 8, "name": 24, "avg_time": 16, "yield_rate": 8, "quantity": 10}
...
def format_sample_list(summaries: List[SampleSummary]) -> str:
    ...
    header = (
        pad("ID", _COLS["id"])
        + pad("시료명", _COLS["name"])
        + pad("평균 생산시간", _COLS["avg_time"])
        + pad("수율", _COLS["yield_rate"])
        + pad("현재 재고", _COLS["quantity"])
    )

    lines = [
        colors.colorize(f"등록 시료 목록 (총 {len(summaries)}종)", colors.HEADER),
    ...
    for s in summaries:
        row = (
            ...
            + pad(f"{s.avg_production_time} min/ea", _COLS["avg_time"])
